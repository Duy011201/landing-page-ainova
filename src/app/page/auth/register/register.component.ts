...
import {AuthService} from '@app/page/auth/module/auth.service';

@Component({
  selector: 'fe-app-register',
  standalone: false,
...
export class RegisterComponent extends BaseComponent implements OnInit {
  ...
  onLogin() {
    ...
    this.showLoading();
    this._authService.apiAuthRegister(this.form.value).subscribe({
      next: (res) => {
        this.hideLoading();
      ...
      error: (err) => {
        this.hideLoading();
        this.toastMessage(this.TOAST_MESSAGE.ERROR, SETTING.MSG.ERROR, err?.message);
